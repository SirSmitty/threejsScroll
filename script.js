...
const outroPosition = outroSection.offsetTop;
...
function animate() {
    if (model) {
        ...
        const scrollProgress = Math.min(currentScroll / outroPosition, 1);

        if (scrollProgress < 1) {
            model.rotation.x = scrollProgress * Math.PI * 2;
            model.rotation.y = scrollProgress * Math.PI * 2;
            // model.rotation.z = (scrollProgress * Math.PI * 2);
        }
